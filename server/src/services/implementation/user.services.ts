...
export class UserService implements IUserService {
  constructor(
    ...
    private enrollmentRepository: any, // Add the enrollment repository
  ...
  async getModulesForCourse(courseId: string, userId: string): Promise<{ modules: IModule[], unlocked: boolean[], completed: boolean[] }> {
    try {
      ...
      const completed: boolean[] = modules.map((mod: any) =>
        mod.lectures.length > 0 && mod.lectures.every((lec: any) => completedLectureIds.has(lec._id.toString()))
      );
  ...
  async getLecturesForModule(moduleId: string, userId: string): Promise<{ lectures: ILecture[], completed: boolean[] }> {
    ...
    const allModuleIds = course.modules.map((m: any) => m._id && m._id.toString());
    ...
    const module = course.modules.find((m: any) => m._id && m._id.toString() === moduleIdStr);
    if (!module) {
    ...
    const completedLectureIds = new Set(progressDocs.map((p: any) => p.lecture.toString()));
    const completed: boolean[] = lectures.map(l => completedLectureIds.has(l._id.toString()));
  ...
  async completeLecture(userId: string, moduleId: string, lectureId: string): Promise<void> {
    ...
    const module = course.modules.find((m: any) => m._id && m._id.toString() === moduleId);
    if (!module) throw new Error('Module not found');
    // Add logging here
    const allLectureIds = module.lectures.map((l: any) => l._id && l._id.toString());
    ...
    const lecture = module.lectures.find((l: any) => l._id && l._id.toString() === lectureId);
    if (!lecture) {
